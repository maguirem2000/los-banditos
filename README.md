...
- **Hot Takes** (Season tab + Home) — on first open each week the site prompts you for your football/fantasy hot take of the week (PIN-signed, posts instantly for the whole league, editable until Thursday kickoff, then it's on the record). League reacts 🔥/🗑️ (one vote each, no self-hype); three weeks later each take opens for 🍷 aged-like-wine / 🥛 aged-like-milk grading, feeding season-long Nostradamus and Clown standings. Same Worker backend (`/take`, `/react`, `/grade`, `/takes` routes); local dev via `npx wrangler dev` + a `.dev.vars` PINS file and the site's `?devapi` flag.
- **Push reminders** (Hot Takes tab → 🔔 Reminders) — real Web Push notifications to each manager's phone (iPhone requires the installed Home-Screen app, iOS 16.4+). Two personalized nudges via Cloudflare cron: Wed 6pm CT if your hot take isn't in, Thu 4pm CT in-season if your picks aren't in before lock — sent only to managers who haven't done the thing. VAPID keys: public in `wrangler.toml`, private in the `VAPID_PRIVATE_KEY` secret; subscriptions in KV; `/push/subscribe`, `/push/unsubscribe`, `/push/test` routes with a send-test button in the UI.
- **Who Ya Rather** (Home + Hot Takes tab) — a weekly auto-generated player poll: three "who would you rather have (dynasty)?" matchups between players within ~5 market-rank spots of each other (never lopsided), locked in for the week server-side so the whole league votes on the same debates. PIN-signed votes, results hidden until you've picked a side. Worker routes `/wyrpairs`, `/wyrvote`, `/wyr`.

Plus: a **Season Preview magazine** (Season tab — auto-written team capsules, projected order, over/under win totals and title odds, generated from Elo + market values + streaks + h2h history), a **This Week in League History** card on Home (one notable moment per past season from the current week number), and a **Live Records Watch** during games (flags any team pacing toward the all-time high/low, blowout or shootout records, and any player chasing the best game ever started).
