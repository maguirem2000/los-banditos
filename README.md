...
- **Trophy Room** — championship banners, career hardware, season-by-season finishes
...
Plus, live on the current season: **playoff odds** (Monte Carlo simulation on the Power Rankings tab), **matchup projections** on the schedule, **grudge-match previews** (all-time series + streaks on every upcoming matchup), an auto-generated **weekly recap**, a **records watch**, and a preseason **kickoff countdown**. The site is an installable PWA — "Add to Home Screen" on your phone.
...
## Refreshing history (once a season)
