/* League hub engine — branding & league shape come from window.LEAGUE_CONFIG (defaults = Los Banditos) */
(function () {
  const D = window.LEAGUE_DATA;
  const E = window.LEAGUE_EXTRAS || {};
  const CFG = Object.assign({
    teams: (D.currentLeague || {}).totalRosters || 8,
    hasHistory: (D.completeSeasons || []).length > 0, // year-one leagues hide history views until they earn them
    apiBase: "https://banditos-picks.lbffl.workers.dev",
    slots: { QB: 1, RB: 2, WR: 2, TE: 1 },
    flex: 3,
    vapidPublic: "BNuvGGc7igXPBD3jWmH5tKSZQwLFLSDMCFvkF7Mj_xJAZoiXkwD-jjcpX5yaNAtrOmKvnRIAhkW85_8CHePRnrs",
    homeUrl: "https://lbffl.com/",
  }, window.LEAGUE_CONFIG || {});
  CFG.fcParams = CFG.fcParams || `isDynasty=true&numQbs=1&numTeams=${CFG.teams}&ppr=1`;
  ...
  /* nav groups: 6 top-level sections, sub-pages as pills. History-driven groups hide in a league's first year. */
  const GROUPS = [
  ...
  ].filter(g => CFG.hasHistory || !["History", "Teams", "Trophies"].includes(g.label));
  ...
  function liveRecordsWatch() {
    if (!LIVE.loaded || !gamedayActive() || !(D.records.highScores || []).length) return "";
    const wk = LIVE.week || 1;
  ...
  function vHome() {
    const c = D.career;
    const hasHist = CFG.hasHistory;
    const hi = hasHist ? D.records.highScores[0] : null, lo = hasHist ? D.records.lowScores[0] : null;
    const lastSeason = hasHist ? D.completeSeasons[D.completeSeasons.length - 1] : null;
    const champUid = hasHist ? D.seasonsData[lastSeason].champion : null;
    const mostTitles = hasHist ? Object.entries(c).sort((a, b) => b[1].champs.length - a[1].champs.length)[0] : null;
    ...
    const tiles = hasHist ? `
      ...
      <div class="tiles">
        <div class="tile gold"><div class="k">Inaugural Season</div><div class="v">${esc(D.currentSeason)}</div><div class="d">every record set this year stands forever</div></div>
        <div class="tile blue"><div class="k">Franchises</div><div class="v">${CFG.teams}</div><div class="d">nobody has won anything yet</div></div>
        <div class="tile red"><div class="k">First ${esc(CFG.sackoName || "Shitter")}</div><div class="v">TBD</div><div class="d">someone has to finish ${ordinal(CFG.teams)}</div></div>
      </div>`;
    ...
    function tableFor(list, title) {
      ...
      return `<div class="card"><h2>${esc(title)}</h2>
        ...
        ${list.map((r, i) => `<tr class="me-row">
          ...
          <td class="num">${r.place === 1 ? '<span class="pill champ">CHAMP</span>' : r.place === CFG.teams ? '<span class="pill sacko">SHITTER</span>' : (r.place ? ordinal(r.place) : "—")}</td>
        </tr>`).join("")}
        </table></div></div>`;
    }
  ...
  function luckCard(season) {
    if (season !== D.currentSeason || !Object.keys(D.luck || {}).length) return "";
  ...
  function vRecords() {
    ...
    const seasonCols = [
      ...
      { h: "Finish", f: r => r.place === 1 ? '<span class="pill champ">CHAMP</span>' : r.place === CFG.teams ? '<span class="pill sacko">SHITTER</span>' : (r.place ? ordinal(r.place) : "—") },
    ];
  ...
  /* ---------- PICK'EM & POWER POLL ---------- */
  const PICKS_API = /[?&]devapi/.test(location.search) ? "http://localhost:8787" : CFG.apiBase;
  ...
  function vPicks() {
    ...
    const submittedNote = pk.subs && !pk.subs.locked && pk.subs.submitted
      ? `<p class="note">${pk.subs.submitted.length}/${CFG.teams} in: ${pk.subs.submitted.map(nameOf).join(", ") || "nobody yet"}</p>` : "";
  ...
  function htHomeCard() {
    ...
    return `<div class="card"><h2>🌶️ Hot Takes <span class="tag">week ${wk} · ${weekTakes.length}/${CFG.teams} filed · <a href="#/takes">archive &amp; grades →</a></span></h2>
  ...
  function vPreview() {
    ...
    const blurb = u => {
      const st = lastS ? D.seasonsData[lastS].standings.find(x => x.uid === u) : null;
      const bits = [];
      if (!lastS) bits.push(`year one — no history, no baggage, no excuses`);
      if (st) {
        if (st.place === 1) bits.push(`the defending champs run it back`);
        else if (st.place === CFG.teams) bits.push(`fresh off the Shitter, there is nowhere to go but up`);
  ...
  function loadTradeValues() {
    ...
    TF.pending = true;
    fetch(`https://api.fantasycalc.com/values/current?${CFG.fcParams}`)
      .then(r => r.json())
      .then(rows => {
        rows.forEach(r => {
  ...
  function tradeModel() {
    ...
    const SLOTS = CFG.slots, FLEX = CFG.flex;
    const teams = {}, uids = [];
  ...
  function vTradeFinder() {
    ...
    return `
      ...
      <p class="footnote">Market values: FantasyCalc dynasty (1QB · ${CFG.teams}-team · PPR), refreshed on every page load. Win-Now = FantasyCalc redraft value, same format. 30-Day = roster-wide value trend. K/DEF excluded.</p>`;
  }
  ...
  function playoffOdds() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const spots = D.currentLeague.playoffTeams || 6;
      const hasDivs = Object.values(divOf).some(Boolean);
      const divWinners = hasDivs ? [1, 2].map(d => rank.find(u => divOf[u] === d)).filter(Boolean) : [];
      const wild = rank.filter(u => !divWinners.includes(u)).slice(0, spots - divWinners.length);
    ...
    const finishGrid = `<div class="table-scroll"><table>
      ...
      ${Object.keys(D.career).sort((a, b) => (seatOf[a] || 9) - (seatOf[b] || 9)).map(uid => {
        ...
        return `<tr class="me-row"><td>${mgrChip(uid)}</td>${D.completeSeasons.map(s => {
          ...
          const style = p === 1 ? 'color:var(--gold-bright);font-weight:700' : p === CFG.teams ? 'color:var(--red);font-weight:700' : "";
  ...
  function vShame() {
    ...
    const seasonCols = [
      ...
      { h: "Finish", f: r => r.place === CFG.teams ? '<span class="pill sacko">SHITTER</span>' : (r.place ? ordinal(r.place) : "—") },
    ];
    ...
    const boardHtml = boards.map(b => {
      ...
      return `<div class="card"><h2>${b.rounds}-Round ${b.type === "snake" ? "Snake" : b.type} Draft <span class="tag">${b.picks.length} picks</span></h2>
        ...
        ${b.picks.map(p => `<tr class="me-row">
          <td class="num">${p.round}.${String(p.no - (p.round - 1) * CFG.teams).padStart(2, "0")}</td>
  ...
  function openManager(uid) {
    ...
    const finishes = seasons.filter(s => D.completeSeasons.includes(s)).map(s => {
      ...
      return `<tr><td>${s}</td><td>${esc(teamOf(uid, s))}</td><td class="num">${st.wins}-${st.losses}</td>
        ...
        <td class="num">${st.place === 1 ? '<span class="pill champ">CHAMP</span>' : st.place === CFG.teams ? '<span class="pill sacko">SHITTER</span>' : ordinal(st.place)}</td></tr>`;
    }).join("");
  ...
  /* ---------- router / render ---------- */
  function render() {
    const view = VIEWS[state.view] && GROUPS.some(g => state.view in g.views) ? state.view : "home";
  ...
  $("#league-sub").textContent = D.seasons.length > 1
    ? `Dynasty · ${D.seasons.length} seasons · est. ${D.seasons[0]} · updated ${D.generatedAt}`
    : `Dynasty · inaugural ${D.currentSeason} season · updated ${D.generatedAt}`;
  nav();
  loadLive();
  loadTakes();
  loadWyr();
